...
module.exports = function(json) {
  ...
  list.redo = function() {
    ...
    internal.makeChange(internal.changes[internal.currentIndex][1]);
    internal.currentIndex += 1;
    return true;
  };
  ...
  list.syncWithServer = function() {
    ...
    if (internal.serverIndex <= internal.currentIndex)
      changes = internal.changes.slice(
        internal.serverIndex,
      ...
      changes = internal.changes.slice(
        internal.currentIndex,
        internal.serverIndex
      ).map(x => internal.invertChange(x[1]));
  ...
  internal.makeChange = function(change) {
    switch (change[0]) {
    case "addTask":
      const addID = change[1];
      const addArgs = change[2];
  ...
  internal.undoChange = function(change) {
    ...
    internal.uuid = uuid;
    internal.makeChange([uuid, internal.invertChange(changeObj)][1]);
  };

  internal.invertChange = function(change) {
    switch (change[0]) {
    case "addTask":
      return ["removeTask", change[1]];
    }
  };
