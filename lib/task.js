...
const Task = function(a, b, c, d) {
  ...
  if (typeof a === "undefined") {
    ...
    if (typeof a === "string") {
      ...
      if (typeof json.intervals !== "undefined")
        for (const interval of json.intervals)
          task.intervals.push(Interval(interval));
      ...
      if (typeof json.categories !== "undefined")
        for (const key of explicitPropertiesOf(json.categories))
          task.categories.set(key, json.categories[key]);
    }
  } else {
  ...
  task.firstSubtask = function() {
    if (task.subtasks.length === 0)
      return task.description;

    else
      return task.subtasks[0].firstSubtask();
  };
  ...
  task.toJSON = function() {
    ...
    if (task.categories.size > 0) {
      json.categories = {};

      for (const [key, value] of task.categories)
        json.categories[key] = value;
    }
